...
static NSString *SPACELINE = @"spaceLineKey";
static NSString *LIMITPLACEHOLDER = @"limitPlaceholder";
static NSString *LIMITSTYLE = @"limitStyle";
static NSString *TEXTFONt = @"textFont";
...
- (void)setTextFont:(UIFont *)textFont{
    objc_setAssociatedObject(self,&TEXTFONt,textFont, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (UIFont *)textFont {
    return objc_getAssociatedObject(self, &TEXTFONt) == nil ? [UIFont systemFontOfSize:13.] : objc_getAssociatedObject(self, &TEXTFONt);;
}
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
- (void)setSpaceLine:(NSNumber *)spaceLine {
    objc_setAssociatedObject(self, &SPACELINE, spaceLine, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSNumber *)spaceLine {
    return objc_getAssociatedObject(self, &SPACELINE);
}

//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
- (void)setLimitStyle:(NSString *)limitStyle {
    objc_setAssociatedObject(self,&LIMITSTYLE,limitStyle, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
- (NSString *)limitStyle{
    return objc_getAssociatedObject(self, &LIMITSTYLE);
}
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
- (void)setLimitPlaceholder:(NSString *)limitPlaceholder {
    objc_setAssociatedObject(self,&LIMITPLACEHOLDER,limitPlaceholder, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    dispatch_async(dispatch_get_main_queue(), ^{
        self.wordCountLabel.text = limitPlaceholder;
    });
}
- (NSString *)limitPlaceholder{
    return objc_getAssociatedObject(self, &LIMITPLACEHOLDER);
}

//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
-(void)setLimitMargin:(UIEdgeInsets)limitMargin{
    dispatch_async(dispatch_get_main_queue(), ^{
        self.wordCountLabel.frame = CGRectMake(0, CGRectGetHeight(self.frame) - 20 - limitMargin.bottom, CGRectGetWidth(self.frame) -limitMargin.right, 20);
        self.contentInset = UIEdgeInsetsMake(0, 0, 20 + limitMargin.bottom, 0);
...
- (void)setPlaceHolderLabel:(NSString *)placeholder {
    ...
    self.placeholderLabel.frame = CGRectMake(7.0, 7.0, rect.size.width, rect.size.height);
...
- (void)textViewChanged:(NSNotification *)notification {
    ...
    if (self.limitLength) {//字数限制
        ...
        if(self.limitStyle){
            NSMutableString *str = self.limitStyle.mutableCopy;
            NSRange rang = [str rangeOfString:@"##"];
            if(rang.location != NSNotFound){
                NSUInteger leng = (self.limitLength.unsignedIntValue - self.text.length);
                if(leng <= 0){
                    str = @"已是最大字数".mutableCopy;
                }else{
                   [str replaceCharactersInRange:rang withString:@(self.limitLength.unsignedIntValue - self.text.length).stringValue];
                }
            }
            self.wordCountLabel.text = str;
        }else{
    ...
    if(self.spaceLine){
        ...
        //调整行间距
        paragraphStyle.lineSpacing= self.spaceLine.floatValue;
        NSDictionary *attriDict =@{
            NSParagraphStyleAttributeName:paragraphStyle,
            NSFontAttributeName:self.font?self.font:[UIFont systemFontOfSize:17],
            NSForegroundColorAttributeName:self.textColor?self.textColor:[UIColor blackColor]
                                   
        };
        [attStr addAttributes:attriDict range:NSMakeRange(0, self.text.length)];
        self.attributedText = attStr;
        if (self.infoBlock) {
               self.infoBlock(self.attributedText.string, self.frame.size);
           }
    }else{
        if (self.infoBlock) {
...
- (void)textViewBeginEditing:(UITextView *)text{
