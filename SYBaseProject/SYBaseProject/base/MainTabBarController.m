...
//#import "SYStoreManagerViewController.h"
...
- (void)viewDidLoad {
    ...
    //获取图片服务器
    [ShareRequest shareRequestDataWithAppendURL:@"/other/other/upload_host" Params:nil IsShowHud:NO IsInteract:NO Complete:^(NSDictionary *dic) {
        
        NSString *hostStr = dic[@"host"];
        if (kStringIsEmpty(hostStr)) {
            hostStr = @"http://qiniu.lovelovepets.com/";
        }
        [[NSUserDefaults standardUserDefaults]setObject:hostStr forKey:@"KNnddkjPetImageSrever"];
        [[NSUserDefaults standardUserDefaults]synchronize];
        
    } Fail:^(NSError *error) {
        [[NSUserDefaults standardUserDefaults]setObject:@"http://qiniu.lovelovepets.com/" forKey:@"KNnddkjPetImageSrever"];
        [[NSUserDefaults standardUserDefaults]synchronize];
    }];
    
    //获取客服微信号
    [ShareRequest shareRequestGetDataWithAppendURL:@"/user/about_user/serve_wechat" Params:nil IsShowHud:NO IsInteract:NO Complete:^(NSDictionary *dic) {
        
        NSString *serve_wechat = dic[@"data"][@"serve_wechat"];
        [[NSUserDefaults standardUserDefaults]setObject:kStringIsEmpty(serve_wechat)?@"":serve_wechat forKey:@"KPetSystemWechatCount"];
        [[NSUserDefaults standardUserDefaults]synchronize];
...
- (NSArray *)viewControllersForTabBar {
    ...
    SYHomePageViewController *firstViewController = [[SYHomePageViewController alloc]init];

    SYRecordManagerViewController *secondViewController = [[SYRecordManagerViewController alloc] init];

    SYCourseManagerViewController *thirdViewController = [[SYCourseManagerViewController alloc] init];

//    SYStoreManagerViewController *fourthViewController = [[SYStoreManagerViewController alloc] init];
    
    SYMainViewController *fiveVC = [[SYMainViewController alloc] init];
    
    NSArray *viewControllers = @[
        [[baseNavigationController alloc]initWithRootViewController:firstViewController],
                                 [[baseNavigationController alloc]initWithRootViewController:secondViewController],
                                 [[baseNavigationController alloc]initWithRootViewController:thirdViewController],
//                                 [[baseNavigationController alloc]initWithRootViewController:fourthViewController],
        [[baseNavigationController alloc]initWithRootViewController:fiveVC]
                                 ];
...
- (NSArray *)tabBarItemsAttributesForTabBar {
    ...
    NSDictionary *firstTabBarItemsAttributes = @{
        ...
        CYLTabBarItemImage : [UIImage imageNamed:@"home_nomal"],  /* NSString and UIImage are supported*/
    ...
    NSDictionary *secondTabBarItemsAttributes = @{
        ...
        CYLTabBarItemImage : [UIImage imageNamed:@"record"],
        CYLTabBarItemSelectedImage : @"record_on",
    };
    
    NSDictionary *thirdTabBarItemsAttributes = @{
        CYLTabBarItemTitle : @"课程",
        CYLTabBarItemImage : [UIImage imageNamed:@"course_off"],
        CYLTabBarItemSelectedImage : @"course_on",
        
    };
//    NSDictionary *fourthTabBarItemsAttributes = @{
//        CYLTabBarItemTitle : @"商城",
//        CYLTabBarItemImage :[UIImage imageNamed:@"shop_off"],
//        CYLTabBarItemSelectedImage : @"shop_on",
//        
//    };
    
    NSDictionary *fiveTabBarItemsAttributes = @{
        CYLTabBarItemTitle : @"我的",
        CYLTabBarItemImage :[UIImage imageNamed:@"my_off"],
        CYLTabBarItemSelectedImage : @"my_on"
    };
...
- (void)customizeTabBarAppearance {
    ...
    normalAttrs[NSForegroundColorAttributeName] = KUIColorFromRGB(0x333333);
    normalAttrs[NSFontAttributeName] = [UIFont fontWithName:@"" size:14];
    
    // set the text color for selected state
    // 选中状态下的文字属性
    NSMutableDictionary *selectedAttrs = [NSMutableDictionary dictionary];
    selectedAttrs[NSForegroundColorAttributeName] = KUIColorFromRGB(0x333333);
    normalAttrs[NSFontAttributeName] = [UIFont fontWithName:@"" size:14];
